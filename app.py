...
if option=='API認証とデータ取得':
    ...
    if uploaded_file is not None:
        ...
        st.write('認証が完了しました')
        username = st.text_input('ユーザー名を入力して下さい。', '03Imanyu')
...
if option=='データ分析':
    ...
    if uploaded_file is not None:
        ...
        df['時間'] = df['時間'].dt.tz_convert('Asia/Tokyo')#JTSに変換 → .dt.tz_convert('Asia/Tokyo')

        df['時刻'] = df['時間'].dt.hour
        time_df = df[['いいね', '時刻']]#いいね数と時刻のみを抽出
        time_df = time_df.sort_values(by=['時刻'], ascending=True)#並べ替え　時刻を若い順に並べ替える
        grouped = time_df.groupby('時刻')#時刻毎に集計する（グループ毎に生成する）

        mean = grouped.mean()#グループ毎の平均
        mean.columns = ['いいね（平均値）']
        size = grouped.size()
        ...
        base = alt.Chart(result, title='時間帯毎の傾向').encode(x='時刻:O')
        ...
        grouped_fav = df.groupby('いいね評価')#等級毎の平均文字数を表でまとめる
        ...
        grades = ['A', 'B', 'C', 'D', 'E']
        for grade in grades:
            _df = df[df['いいね評価'] == grade] #ここにはいいね評価Aのみが入っている　
            num_tweet = len(_df) #いいね評価Aツイート数
            #特定の等級の
            txt = ' '.join(_df['ツイート本文'].to_list()).replace('http://t.co/', '')
            parts = wakati.parse(txt)


            words = []
            for part in parts.split('\n'):
                if '名詞' in part: #名詞が付いていたら
                    word = part.split('\t')[0]
                    words.append(word)
